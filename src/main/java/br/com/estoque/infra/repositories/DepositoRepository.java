...
public class DepositoRepository implements IDepositosService {
    ...
    public void deleteDepositos(Integer id) {
        ...
        try {
            String query = "DELETE ES_DEPOSITOS WHERE ID =" + id;
            Connection connection = dataSource.getConnection();
    ...
    public void createDepositos(Depositos depositos) {
        try {
            ...
            String query = "DECLARE V_ID ES_DEPOSITOS.ID%TYPE;BEGIN PCKG_CRUD.INSERE_DEPOS_PCKG('"
                    + depositos.getNOM_DEPOS() + "', V_ID);END;";
            Connection connection = dataSource.getConnection();
            PreparedStatement ps = connection.prepareStatement(query);
            ps.executeQuery();
