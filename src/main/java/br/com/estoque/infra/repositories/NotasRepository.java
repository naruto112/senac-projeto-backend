...
public class NotasRepository implements INotasService {
    ...
    public void insertNota(Notas nota) {
        try {
            String query = "DECLARE V_ID ES_NOTA_FISCAL.ID%TYPE;BEGIN PCKG_CRUD.INSERE_NOTA_PCKG(" + nota.getNUM_NOTA()
                    + "," + nota.getID_FORNEC() + " ," + nota.getSTA_ENTRADA_SAIDA() + ", V_ID);END;";
            Connection connection = dataSource.getConnection();
